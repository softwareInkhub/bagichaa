...
export default function OrdersManagement() {
  ...
  const filteredOrders = orders.filter(order => {
    ...
    const matchesSearch = !searchTerm || 
      ...
      (order.address?.fullName || order.customerName || '').toLowerCase().includes(searchTerm.toLowerCase())
    ...
    switch (sortBy) {
      ...
      case 'customer':
        const nameA = a.address?.fullName || a.customerName || ''
        const nameB = b.address?.fullName || b.customerName || ''
            ...
            <div className="relative">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-gray-600 truncate">{order.address?.city || 'N/A'}, {order.address?.state || 'N/A'}</span>
                </div>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                    ...
                    <div className="flex items-start space-x-2">
                      ...
                      <div>
                        <p className="font-medium">{selectedOrder.address?.fullName || selectedOrder.customerName || 'N/A'}</p>
                        <p>{selectedOrder.address?.addressLine1 || selectedOrder.address?.street || 'Address not available'}</p>
                        {selectedOrder.address?.addressLine2 && <p>{selectedOrder.address.addressLine2}</p>}
                        <p>{selectedOrder.address?.city || 'N/A'}, {selectedOrder.address?.state || 'N/A'} - {selectedOrder.address?.pincode || 'N/A'}</p>
                  ...
                  <div className="space-y-3">
                    {selectedOrder.items.map((item, index) => (
                      <div key={index} className="flex items-center space-x-4 p-3 bg-gray-50 rounded-lg">
                        ...
                        <div className="text-right">
                          <p className="font-semibold">{formatCurrency(item.price * (item.qty || item.quantity || 1))}</p>
